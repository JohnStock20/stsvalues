// Archivo: netlify/functions/giveaways-manager.js (VERSIÓN FINAL)
// Propósito: Gestionar todas las operaciones relacionadas con los sorteos, con soporte para prize pools complejos.
...
exports.handler = async (event) => {
  ...
  // Usamos un switch para dirigir la petición al manejador correcto
  switch (httpMethod) {
    case 'GET':
...
async function handleGetGiveaways(event) {
  ...
  try {
    ...
    // Se incluye el prize_pool y el conteo de participantes.
...
async function handleCreateGiveaway(event) {
  ...
  try {
    ...
    // Validación de datos
    if (!prize_pool || !Array.isArray(prize_pool) || prize_pool.length === 0 || !start_time || !end_time) {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO giveaways (prize_pool, start_time, end_time, created_by) 
         VALUES ($1, $2, $3, $4)`,
