...
function handleNavigation(view, payload) {
    ...
    switch (view) {
        ...
        case 'selection':
        default:
            UI.showView('selection', swordUpdateInterval);
            break;
    }
}
    ...
    UI.inputs.searchBar.addEventListener('input', () => {
        ...
        if (results.length > 0) {
            results.forEach(reward => {
                ...
                item.querySelector('.reward-stats').style.display = 'none'; // No mostrar stats en la búsqueda
                item.addEventListener('click', () => {
                    handleNavigation('swordDetails', { sword: reward, source: sourceInfo });
...
function initializeApp() {
    ...
    document.getElementById('details-to-selection-btn').addEventListener('click', () => handleNavigation('selection'));
    document.getElementById('sword-to-details-btn').addEventListener('click', () => {
        if (navigationContext.type === 'case') {
            handleNavigation('caseDetails', navigationContext.id);
        } else {
    ...
    document.getElementById('other-prev-btn').addEventListener('click', () => {
        if (appState.currentPage > 1) {
            ...
            UI.renderOtherSwords(appState, handleNavigation);
        }
    });
    ...
    UI.renderCaseSelection(handleNavigation);
    UI.renderOtherSwords(appState, handleNavigation);
