...
import { findSwordById, formatLargeNumber, formatTimeAgo,
getPrizeItemHtml, formatHours } from './utils.js';
...
export const dom = {
	...
	containers: {
        ...
        titleDetailsInner: document.getElementById('title-details-view-inner'), // Para el nuevo layout
        ...
        // Contenedor del panel derecho de detalles de la caja
        infoColumnInner: document.querySelector('.info-column-inner'),
	},
	buttons: {
...
export function showView(viewName) {
	Object.values(dom.views).forEach(view => { if (view)
...
function getCurrencyHTML(currencyKey, price) {
	if (currencyKey === 'cooldown') return `<span
class="currency-text">Free (Every ${price} hr)</span>`;
	const currency = appData.currencies[currencyKey];
	if (currency.icon) return `<img src="${currency.icon}"
alt="${currency.name}" class="currency-icon"> <span
class="value">${price.toLocaleString()}</span>`;
	return `<span class="currency-text">${currency.name}</span> <span
class="value">${price.toLocaleString()}</span>`;
}
...
export function renderCaseSelection(navigateTo) {
	...
	Object.keys(appData.cases).forEach(caseId => {
        ...
        // Contenido va dentro del ::after que ahora es el fondo
		caseItem.innerHTML = `
...
export function createRewardItem(reward, source, navigateTo) {
    ...
    // Asigna el color de borde dinámico para que el CSS lo use
    const rarityInfo = findRarityInfo(reward.rarity);
    item.style.setProperty('--border-dynamic-color', rarityInfo.color);
    if (rarityInfo.shadow) {
        item.style.setProperty('--shadow-color', rarityInfo.shadow);
    }

	item.innerHTML = createRewardItemHTML(reward, source); // El contenido va dentro del div principal
	
    item.addEventListener('click', () => navigateTo('swordDetails', {
	sword: reward, source }));
	return item;
}
...
function createRewardItemHTML(reward, source) {
    ...
    const valueDisplayHTML = (typeof reward.value === 'string' &&
    reward.value.toUpperCase().startsWith('O/C'))
    ? `<span class="value-oc" title="Owner's Choice">O/C</span>`
    : formatLargeNumber(parseValue(reward.value));
    
    // El contenido ahora se genera para ir dentro del .reward-item > *
    return `
...
// Función auxiliar para obtener colores de rareza
function findRarityInfo(rarity) {
    const root = document.documentElement;
    const style = getComputedStyle(root);
    let colorVar = `--${rarity}`;
    if (rarity.endsWith('gradient')) {
         colorVar = `--${rarity}`;
    } else if (style.getPropertyValue(colorVar + '-gradient').trim()) {
        colorVar = `--${rarity}-gradient`;
    }
    const color = style.getPropertyValue(colorVar).trim();
    const shadow = `rgba(${parseInt(color.slice(1,3),16)}, ${parseInt(color.slice(3,5),16)}, ${parseInt(color.slice(5,7),16)}, 0.7)`;
    return { color, shadow };
}
...
export function renderCaseDetails(caseId, navigateTo) {
	...
	dom.containers.infoColumnInner.innerHTML = `
        <img id="details-case-image" src="${data.image}" alt="${data.name}">
...
function parseAndSetDescription(element, text, navigateTo) {
	element.innerHTML = '';
	if (!text) { element.textContent = 'No description available.';
	return; }
	...
	while ((match = regex.exec(text)) !== null) {
		...
		if (type === 'case') {
			...
			if (linkData) {
				...
				link.onclick = (e) => { e.preventDefault();
				navigateTo('swordDetails', linkData); };
			}
		}
...
export function renderSwordDetails(sword, sourceInfo, navigateTo, onNewInterval) {
	...
	swordInfoCard.className = `sword-info-card ${sword.rarity}`;
    
    // Aplicar color de borde dinámico
    const rarityInfo = findRarityInfo(sword.rarity);
    swordInfoCard.style.setProperty('--border-dynamic-color', rarityInfo.color);
    
    // Contenido dentro de la tarjeta
    swordInfoCard.innerHTML = `
        <div id="sword-demand-indicator"></div>
        <div id="sword-details-image-container"><img src="${sword.image}" alt="${sword.name}"></div>
        <h3 id="sword-details-name">${sword.name}</h3>
        <p id="sword-details-description"></p>
    `;
    
    // Contenedores de stats
    const statsContainer = document.querySelector('.sword-details-content .sword-stats-container');
    statsContainer.innerHTML = `
        <div class="stat-box">
            <div class="stat-box-inner">
                <h4>VALUE</h4>
                <p>${formatLargeNumber(parseValue(sword.value))}</p>
                <span class="sub-text" id="sword-details-updated"></span>
            </div>
        </div>
        <div class="stat-box">
            <div class="stat-box-inner">
	...
	const fullDescription = sword.description || (sourceInfo.id ? `This
	sword is obtainable from the [case:${sourceInfo.id}].` : 'No origin specified.');
    ...
    document.getElementById('sword-details-more').innerHTML = `
		...
		Rarity - <span class="rarity-text ${sword.rarity} ${rarityInfo.color.includes('gradient') ? 'gradient' : ''}" style="background-image: ${rarityInfo.color.includes('gradient') ? rarityInfo.color : 'none'}">${sword.rarity}</span>
    `;
...
// ... El resto del archivo ui.js sigue aquí sin cambios en la lógica de calculadora, etc.
// ... (código de renderResultsTable, renderSimulationLoot, renderHuntResult, renderProfitGraph, etc.)
// ... (código de renderizado de títulos, sorteos y admin)
...
export function clearCalculator(appState) {
	...
	dom.controls.standard.style.display = isGraphMode ? 'none' :
	'flex';
...
export function renderResultsTable(data, appState) {
	dom.containers.resultsTable.innerHTML = '';
	const quantity = data.quantityOverride ||
	parseInt(dom.inputs.caseQuantity.value, 10) || 1;
	const resultClass = data.result >= 0 ? 'profit' : 'loss';
	const resultSign = data.result >= 0 ? '+' : '';
	const profitPerCase = data.result / quantity;
	const profitPercentage = data.totalCost > 0 ? (data.result /
	data.totalCost) * 100 : (data.result > 0 ? Infinity : 0);
	const profitPercentageDisplay = isFinite(profitPercentage) ?
	...
	const tableHTML = `
		<table id="results-table">
			<thead>
				<tr>
					<th>${appState.calculatorMode === 'theoretical' ?
					'Expected Value' : 'Total Value'}</th>
					<th>Total Cost</th>
					<th>Net Result</th>
					<th>Result/Case</th>
					<th>Profit %</th>
...
export function renderSimulationLoot(wonItems) {
	...
	for (const rewardId in wonItems) {
		...
		if (rewardData) {
            const rarityInfo = findRarityInfo(rewardData.rarity);
			listHTML += `<li>${wonItems[rewardId]}x <span class="rarity-text ${rewardData.rarity} ${rarityInfo.color.includes('gradient') ? 'gradient' : ''}" style="background-image: ${rarityInfo.color.includes('gradient') ? rarityInfo.color : 'none'}">${rewardData.name}</span></li>`;
		}
	}
	listHTML += '</ul>';
...
export function renderProfitGraph(results, MAX_GRAPH_SECTIONS) {
	...
	const xScale = (cases) => padding.left + ((cases - minCases) /
	(maxCases - minCases)) * chartWidth;
	...
	results.forEach(d => {
		...
		circle.addEventListener('mouseover', () => {
			...
			`${sign}${d.value.toFixed(2)}%` :
			`${sign}${formatLargeNumber(d.value)}`;
	...
	const labelIndices = numLabels <= 1 ? [0] : Array.from({ length:
	numLabels }, (_, i) => Math.floor(i * (results.length - 1) / (numLabels - 1)));
	if (results.length > 0) {
...
export function renderTitlesPage(titlesData, selectedKey, onSelect, onEquip) {
	...
	titlesData.forEach(title => {
        ...
        inner.className = 'title-list-item-inner';
		inner.innerHTML = `
            <span class="title-name ${styleInfo.style.includes('gradient') ? 'gradient' : ''}"
                style="${styleInfo.style.includes('gradient') ?
                `background-image: ${styleInfo.style}` : `color: ${styleInfo.style}`}">
                ${styleInfo.text}
            </span>
...
function renderTitleDetails(title, onEquip) {
    ...
    const innerContainer = dom.containers.titleDetailsInner;

	if (!title) { 
        innerContainer.innerHTML = '<p>Select a title from the list to see its details.</p>'; 
        return; 
    }
	const styleInfo = titleStyles[title.key] || titleStyles['player'];
...
export function renderGiveawayPage(giveaways, recentWinners, currentUser, onJoin, onHost) {
	...
	if (currentUser && ['owner', 'tester'].includes(currentUser.role)) {
        ...
        hostBtn.className = 'auth-button'; // Reutilizamos clase existente
...
function renderActiveGiveaway(giveaway, currentUser, onJoin) {
	...
	if (!giveaway) {
		container.innerHTML = `<div class="giveaway-card"><div class="giveaway-card-inner"><h2>There are no active giveaways right now. Check back soon!</h2></div></div>`;
		return;
	}
...
function renderUpcomingGiveaways(giveaways) {
	...
	giveaways.slice(0, 5).forEach(gw => {
		...
		${p.type === 'currency' ? (appData.currencies[p.id] ?
		appData.currencies[p.id].name : p.id) : (findSwordById(p.id) ?
		findSwordById(p.id).sword.name : p.id)}`).join(' + ');
		list.innerHTML += `
			<div class="upcoming-giveaway-item">
				<span class="prize">${prizeText}</span>
				<span class="time">Starts: ${new
				Date(gw.start_time).toLocaleString()}</span>
			</div>`;
	});
...
export function openGiveawayModal() {
	...
	setTimeout(() =>
	dom.modals.createGiveaway.classList.add('visible'), 10);
}
...
export function updateProfileHeader(userData) {
    ...
    if (styleInfo.style.includes('gradient')) {
        ...
        userProfileNav.style.background = styleInfo.style;
    } else {
