// Archivo: utils.js (MODIFICADO)
// Propósito: Contiene funciones de utilidad puras y reutilizables en toda la aplicación.
...
// NUEVA FUNCIÓN para formatear la fecha de desbaneo
export function formatBanTime(expiresAtISO) {
    if (!expiresAtISO) {
        return 'Permanent';
    }
...
export function getPrizeItemHtml(prize) {
  ...
  if (prize.type === 'currency') {
    ...
    return `
      ...
      <span>${prize.amount > 1 ? `${prize.amount}x ` : ''}${sword.name}</span>`;
  }
}